...
## 3. The Demo Project
The next step is to describe our demo example. We're going to create a very simple project for measuring temperature and printing the result.
...
Note: HAL traits are provided by the `embedded-hal` crate and do not depend on hardware choice. That's why on the diagram the traits themselves are not included in the `stm32-target` package, but trait implementations are included. 
  
All hardware-dependent code is in the `stm32-target` package:
- `.cargo/config.toml` - local Cargo configuration file, where we specify a build target for cross-compilation 
```toml
[build]
...
But we will take a simpler approach and use the `embedded-hal-mock` crate with ready-made mock implementations for `embedded-hal` traits. For example, mocking I2C is done with just a couple of lines.
```rust
let expected = vec![Transaction::write_read(0x77, vec![0xD0], vec![0x42])];
let mut i2c = i2c::Mock::new(&expected);
```

Let's see an example of a test with a mock.
...
Ironically, the main strength of unit tests - testing code in isolation - is also their limitation. A big part of application complexity lies not in the components themselves, but in component interactions. This part we'll cover in the next section - integration tests. The second limitation is that they don't test real hardware.
...
A large part of the stability of the embedded system depends on the hardware part (proper initialization and configuration). We tried to abstract away the hardware part, but a small misconfiguration in initialization can make the whole system misbehave.

![Target tests](img_5_target_tests.png)

We already have a place that is not abstracted from the hardware - `stm32-target` package, and that will be the place for the tests that target specific hardware. 

Purpose of these tests:
  - ensure initialization or some part of the initialization phase works as expected, and changes in code do not introduce regressions
...
1. Add a dev dependency to the package `Cargo.toml`
```toml
[dev-dependencies]
embedded-test = { version = "0.7.1", features = ["defmt"] }
```
2. Disable the standard test runner for every test file in the package `Cargo.toml`; we have just one at the moment (the `initialization_tests.rs` file)
```toml
[[test]]
name = "initialization_tests"
harness = false
```
3. Add an instruction for the linker in the `build.rs` file
```rust
fn main() {
...
```
1. Test will run on a target device - `#![no_std]` is required, and we can't use the standard library for these tests
...
4. Test validates the subset of operations of the initialization phase of the application - `fn bme280_connection_test(peripherals: Peripherals)`
...
```
And we will have a clean test report
```
...
Target tests have some limitations:
   1. They depend on specific hardware; we can't write a single test for all the platforms. And in fact we don't want to do it; in target tests we want to embrace hardware differences and ensure we understand them correctly
   2. They take more time to run, due to target flashing and run on a slow CPU
   3. Harder to set up, requires hardware wiring, can be an issue in a remote environment
   ...
   6. Limited to `core-crate` and no access to Rust standard library and third-party crates requiring `std-crate`.
